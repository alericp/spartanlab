    ...
    subtasks: [
      ...
      },
      {
        ...
        evidence: [
          ...
          '[STEP 23.4] "I Can\'t Train Today" button added to advisory card',
          '[STEP 23.4] Confirmation modal opens with full advisory preview',
          '[STEP 23.4] Modal shows badge, title, summary, recommendation, and reasoning',
    ...
    subtasks: [
      ...
      },
      {
        ...
        evidence: [
          ...
          '[V.V4] Duplicate-apply guard via [V.V4:timestamp] marker in session.adaptationNotes (typed field)',
          '[V.V4-A] Fixed: removed invalid lastModified on AdaptiveProgram, removed intensityReductionProvenance on session (not typed fields)',
  ...
  return {
    ...
    subtasks: [
      ...
      },
      {
        ...
        status: 'COMPLETE',
        evidence: [
          '[W.W8] lib/program/selected-skill-representation-guidance.ts helper created and verified',
          '[W.W8] deriveAllSelectedSkillRepresentations() maps every selected skill to display state',
          '[W.W8] 6 representation states: headline_priority, direct, support, accessory_carryover, underrepresented, unknown',
          '[W.W8] Each state has visibleBadge and explanation text for user clarity',
          '[W.W8] Truth precedence: headline identity → weeklyRepresentation.policies → summaryTruth → fallback',
          '[W.W8] SKILL_CARRYOVER_EXPLANATIONS maps skills to family/carryover explanation text',
          '[W.W8] Centralized thresholds: DIRECT_EXPOSURE_THRESHOLD=2, TOTAL_EXPOSURE_MEANINGFUL_THRESHOLD=3',
          '[W.W8R] AdaptiveProgramDisplay.tsx now imports and consumes deriveAllSelectedSkillRepresentations()',
          '[W.W8R] Old duplicate local corridor removed: SkillWithRepresentation, allSelectedSkillsWithRepresentation, primaryDirectSkills, supportAccessorySkills',
          '[W.W8R] UI renders ALL skills via selectedSkillRepresentations array with getRepresentationStateStyles()',
          '[W.W8] Underrepresented/unknown skills visible with badge suffix instead of hidden',
          '[W.W8] Explanations appear on hover via title attribute for compact UI',
          '[W.W8] No selected skill silently disappears — all are rendered with appropriate state',
          '[W.W8] Advisory-only — no mutation, no generator changes, no storage writes',
          '[W.W8R] Helper is no longer orphaned — directly consumed by Program display component',
        ],
        remainingWork: [],
      },
      {
