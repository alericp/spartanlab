...
import { RPE_QUICK_OPTIONS, type RPEValue, toDisplayRPE } from '@/lib/rpe-adjustment-engine'
...
if (shouldShowLocalFallback) {
  ...
  if (sessionPhase === 'warmup') {
    ...
    // [PPX-R4D] Handle back navigation within warmup - go to previous warmup item
    // At first item, Back is disabled (no unsafe reload)
    const handleWarmupBack = () => {
      if (warmupIndex > 0) {
        setWarmupIndex(prev => prev - 1)
      }
      // At index 0, Back is disabled - no action needed
    }
            ...
            {currentWarmupItem ? (
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  {/* [PPX-R4D] Action row: Back | Skip This | Skip Warm-Up */}
          ...
          <DialogContent className="bg-[#1A1F26] border-[#2B313A] text-[#E6E9EF] w-[calc(100vw-32px)] max-w-[400px] max-h-[85vh] overflow-y-auto">
            ...
            <div className="space-y-3">
              {(() => {
                ...
                const currentSessionCount = currentSessionSets.length
                const lastSessionSet = currentSessionSets[currentSessionSets.length - 1]
                const currentSessionAvgRPE = currentSessionSets.length > 0
                  ? toDisplayRPE(currentSessionSets.reduce((sum, s) => sum + (s.actualRPE || 0), 0) / currentSessionSets.length)
                  : null
      ...
      <Dialog open={adaptiveDetailsOpen !== null} onOpenChange={(open) => !open && setAdaptiveDetailsOpen(null)}>
        <DialogContent className="bg-[#1A1F26] border-[#2B313A] text-[#E6E9EF] max-w-md">
          ...
          <div className="space-y-4 mt-2">
            ...
            {/* [PPX-R7.6B] Live dialog moved to isLiveExecutionPhase return tree - this branch is now unreachable during live */}
            {/* The live dialog is now mounted in the active live return at line ~10401 */}
            {false && adaptiveDetailsOpen === 'live' && (() => {
