...
export function resolveBoundedMutationApplyEligibilityGate(
  ...
  if (
    ...
    return {
      ...
      headline: 'Apply gate blocked by active caution',
      summary: `${activeCautionCount} active caution${activeCautionCount !== 1 ? 's' : ''} must be cleared before apply eligibility can be evaluated. Completed sessions remain protected.`,
      ...
      eligibleOperationCount: 0,
      blockedReasons: [
        `${activeCautionCount} active caution${activeCautionCount !== 1 ? 's' : ''} blocking apply eligibility`,
        'Caution clearance gate not ready',
        'Apply controls remain locked until cautions are resolved',
      ],
      safetyNotes: [
        'Completed sessions protected',
        'No apply controls shown',
        'No mutation possible with active cautions',
      ],
      nextSafeGate: 'Clear active cautions before evaluating apply eligibility',
      ...LOCKED_UI_FLAGS,
      ...LOCKED_WRITE_FLAGS,
      ...LOCKED_SAFETY_FLAGS,
    }
