...
const TRUE_SOURCE_REGISTRY: readonly TrueSourceRegistryEntry[] = [
  ...
  // -------------------------------------------------------------------------
  {
    ...
    maxIntentNotes: 'Method Planner is the most recently repaired corridor; protect it absolutely until MASTER-8B.5',
    nextAllowedAction: 'Safe integration with foundation sources only in MASTER-8B.5',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    currentSourceSummary: 'Doctrine DB foundation exists but not final generator-wired',
    ...
    riskLevel: 'high',
    safeInsertionPoint: 'MASTER_8B_2',
    protectedBehaviors: [],
    forbiddenNow: ['Generator wiring until MASTER-8C'],
    maxIntentNotes: 'Knowledge base is the foundation for max-intent programming; must be built before generator wiring',
    nextAllowedAction: 'Seed schema creation in MASTER-8B.2',
    readiness: 'needs_knowledge_schema',
  },
