-- SpartanLab History System Tables
-- Provides durable storage for program history, workout sessions, and PR tracking
...
CREATE TABLE IF NOT EXISTS workout_session_history (
  ...
  program_history_id TEXT,                   -- Link to program_history.id
  ...
  fatigue_rating INTEGER,                    -- 1-10 scale
...
CREATE TABLE IF NOT EXISTS personal_record_history (
  ...
  workout_session_id TEXT,                   -- Link to workout_session_history.id
  program_history_id TEXT,                   -- Link to program_history.id
...
COMMENT ON TABLE program_history IS 'Durable archive of all program versions with full snapshots';
...
COMMENT ON COLUMN workout_session_history.summary_message IS 'The highlighted message for that training day';
