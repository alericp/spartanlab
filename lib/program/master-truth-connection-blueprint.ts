...
function step22(): BlueprintPhase {
  ...
  return {
    ...
    status: 'PARTIAL',
    nextAction:
      'Step 22.1-22.4 COMPLETE. T.T9-T.T12 complete. T.T13 PARTIAL — build contract fixed, saved-program apply blocked until safe corridor verified. Post-workout proposal queue renders in pre-save completion UI. Second-confirmation UI exists but returns blocked. No automatic mutation. Also pending: T.T7 Program Page advisory preview, T.T8 live workout advisory warning modal.',
    subtasks: [
      {
        id: 'T.T1',
      ...
      // [T.T13] Step 22.5 — Explicit saved-program substitution apply (PARTIAL - build contract fixed, apply blocked)
      {
        id: 'T.T13',
        title: 'Step 22.5 explicit saved-program substitution apply',
        status: 'PARTIAL',
        evidence: [
          'Step 22.5 import/export build contract fixed',
          'ProposalQueueScope type exported',
          'ScopedPostWorkoutSubstitutionProposalQueue type exported',
          'SavedProgramSubstitutionApplyCandidate type exported',
          'SavedProgramSubstitutionApplyConfirmation type exported',
          'SavedProgramSubstitutionApplyResult type exported',
          'buildProposalQueueStorageKey() exported — scoped storage key',
          'saveScopedProposalQueue() exported — persists to sessionStorage',
          'loadScopedProposalQueue() exported — loads from sessionStorage',
          'clearScopedProposalQueue() exported — clears sessionStorage',
          'updateAndPersistProposalStatus() exported — persists dismiss/defer/accepted state',
          'buildSavedProgramSubstitutionApplyCandidate() exported — builds candidate from proposal',
          'buildSavedProgramSubstitutionConfirmation() exported — builds second confirmation',
          'canApplySavedProgramSubstitution() exported — returns blocked (safe corridor not verified)',
          'applySavedProgramSubstitutionToProgram() exported — returns no_safe_update_corridor',
          'getApplyConfirmationDisplayInfo() exported — confirmation UI display',
          'Second confirmation UI in StreamlinedWorkoutSession post-workout review',
          'Saved-program mutation blocked until safe update corridor is verified',
          'No automatic mutation',
          'No generator changes',
          'No schema changes',
          'No package changes',
          'Step 22.2 current-session-only preserved',
          'Step 22.3 live workout safer-option badge preserved',
          'Step 22.4 proposal queue preserved',
        ],
        remainingWork: [
          'Verify and implement safe saved-program update corridor',
          'Enable actual saved-program mutation after corridor verification',
