...
export async function POST(request: Request) {
  ...
  try {
    ...
    const truthExtraction = extractProgramTruth(canonicalProfileOverride, programInputs, 'regenerate')
    logMaterialInputPresence(truthExtraction.truthContext.materialInputPresence)
    
    // Attach truth explanation to the program
    program = attachTruthExplanation(program, canonicalProfileOverride, 'regenerate')
    
    console.log('[ai-truth-alignment-regenerate]', {
