...
export const ADVANCED_SKILL_SUPPORT_PATTERNS: Record<string, {
  ...
  // ===========================================================================
  // [PHASE 3D REGISTRY-BREADTH-LOCK] Six advanced skill support patterns ADDED.
  //
  // Pre-3D, ADVANCED_SKILL_SUPPORT_PATTERNS only covered: hspu, back_lever,
  // dragon_flag, planche_pushup, one_arm_pull_up, one_arm_chin_up,
  // one_arm_push_up. The selector's `getAdvancedSkillSupportExercises`
  // (program-exercise-selector.ts L2229) returns null for any skill not in
  // this registry — so for an advanced athlete with planche primary +
  // front lever secondary + handstand + v-sit selected, FIVE of those skills
  // had no canonical support recommendations and degraded to generic accessory
  // selection.
  //
  // Every exerciseId below is verified present in lib/adaptive-exercise-pool.ts.
  // No phantom IDs (which would silently drop in the selector when the pool
  // lookup fails). Patterns are ordered by skill-relevance, not alphabetically.
  // ===========================================================================
  ...
  front_lever: {
    ...
    primarySupport: [
      {
        // Static FL holds anchor the position-specific straight-arm strength.
        exerciseIds: ['tuck_fl', 'adv_tuck_fl', 'one_leg_fl', 'straddle_fl'],
        purpose: 'Front lever position strength',
      },
      {
        // Dynamic FL carryover — direct skill expression for advanced athletes.
        exerciseIds: ['fl_rows', 'tuck_front_lever_pull', 'ice_cream_maker'],
        purpose: 'Dynamic FL pulling carryover',
      },
    ],
    secondarySupport: [
      {
        // Loaded vertical pull builds the raw pulling base FL converts from.
        exerciseIds: ['weighted_pull_up', 'archer_pull_up', 'typewriter_pull_up'],
        purpose: 'Loaded pulling foundation',
      },
      {
        // Scapular control under FL load.
        exerciseIds: ['scapular_retraction_hold', 'face_pull'],
        purpose: 'Scapular control',
      },
    ],
    trunkSupport: {
      // Anti-extension is the dominant FL core demand.
      exerciseIds: ['hollow_body', 'hollow_body_rock', 'dragon_flag_tuck', 'dragon_flag_neg'],
      purpose: 'Anti-extension trunk for FL line',
    },
  },
  ...
  planche: {
    ...
    primarySupport: [
      {
        // Static planche holds anchor position strength.
        exerciseIds: ['planche_lean', 'tuck_planche', 'adv_tuck_planche', 'straddle_planche'],
        purpose: 'Planche position strength',
      },
      {
        // Dynamic pseudo-planche pressing for advanced carryover.
        exerciseIds: ['pppu', 'planche_lean_pushup', 'tuck_planche_pushup'],
        purpose: 'Dynamic straight-arm pressing carryover',
      },
    ],
    secondarySupport: [
      {
        // General pushing strength reservoir.
        exerciseIds: ['ring_dip', 'weighted_dip', 'dip'],
        purpose: 'Pressing strength foundation',
      },
      {
        // Wrist tolerance is a known planche limiter.
        exerciseIds: ['wrist_circles'],
        purpose: 'Wrist conditioning for weight bearing',
      },
    ],
    trunkSupport: {
      // Anti-extension + compression for the planche line.
      exerciseIds: ['hollow_body', 'hollow_body_rock', 'compression_pulse'],
      purpose: 'Anterior chain control for planche line',
    },
  },
  ...
  handstand: {
    ...
    primarySupport: [
      {
        // Wall variants build line and balance prerequisites.
        exerciseIds: ['wall_hs_hold', 'wall_handstand_hold', 'handstand_shoulder_taps'],
        purpose: 'Wall-supported handstand line',
      },
      {
        // Vertical pressing develops the overhead strength reservoir.
        exerciseIds: ['pike_pushup', 'pike_pushup_elevated', 'wall_hspu_partial'],
        purpose: 'Vertical pressing strength',
      },
    ],
    secondarySupport: [
      {
        // Free balance work — the actual skill ladder peak.
        exerciseIds: ['freestanding_hs_hold', 'freestanding_handstand_hold'],
        purpose: 'Freestanding balance practice',
      },
      {
        exerciseIds: ['face_pull', 'scapular_retraction_hold'],
        purpose: 'Scapular stability under load',
      },
    ],
    trunkSupport: {
      exerciseIds: ['hollow_body', 'hollow_body_rock'],
      purpose: 'Anti-extension for handstand line',
    },
  },
  ...
  v_sit: {
    ...
    primarySupport: [
      {
        // L-sit ladder is the prerequisite chain for v-sit.
        exerciseIds: ['tuck_l_sit', 'single_leg_l_sit', 'l_sit_skill', 'advanced_l_sit'],
        purpose: 'L-sit prerequisite chain',
      },
      {
        // Direct v-sit / manna progression rows.
        exerciseIds: ['v_sit_progression', 'v_sit_hold', 'manna_progression'],
        purpose: 'Direct v-sit / manna expression',
      },
    ],
    secondarySupport: [
      {
        // Compression accumulation work.
        exerciseIds: ['hanging_leg_raise', 'toes_to_bar', 'compression_pulse'],
        purpose: 'Compression strength accumulation',
      },
      {
        // Hamstring + pancake mobility is a real v-sit limiter.
        exerciseIds: ['pike_stretch', 'pancake_stretch', 'active_pancake_lean'],
        purpose: 'Pike & pancake mobility',
      },
    ],
    trunkSupport: {
      exerciseIds: ['hollow_body', 'compression_pulse'],
      purpose: 'Active compression trunk',
    },
  },
  ...
  l_sit: {
    ...
    primarySupport: [
      {
        // L-sit ladder, low → high.
        exerciseIds: ['tuck_l_sit', 'single_leg_l_sit', 'l_sit_core', 'l_sit_skill'],
        purpose: 'L-sit progression ladder',
      },
      {
        // Compression accumulation rows.
        exerciseIds: ['hanging_leg_raise', 'toes_to_bar', 'compression_pulse'],
        purpose: 'Compression strength accumulation',
      },
    ],
    secondarySupport: [
      {
        // Advanced variants (straddle / hanging) for athletes already past
        // the floor l-sit threshold.
        exerciseIds: ['advanced_l_sit', 'hanging_l_sit'],
        purpose: 'Advanced l-sit variants',
      },
      {
        // Dip support is the shoulder-depression endurance limiter.
        exerciseIds: ['dip', 'ring_dip'],
        purpose: 'Shoulder depression support',
      },
    ],
    trunkSupport: {
      exerciseIds: ['hollow_body', 'compression_pulse'],
      purpose: 'Active compression core',
    },
  },
  ...
  muscle_up: {
    ...
    primarySupport: [
      {
        // Pulling height & speed prerequisites.
        exerciseIds: ['chest_to_bar_pull_up', 'explosive_pull_up', 'weighted_pull_up'],
        purpose: 'Explosive pulling height',
      },
      {
        // Transition / negative work.
        exerciseIds: ['muscle_up_negative', 'muscle_up_negative_skill', 'muscle_up_transition_drill'],
        purpose: 'Transition control',
      },
    ],
    secondarySupport: [
      {
        // Dip strength is the second of the two MU components.
        exerciseIds: ['dip', 'ring_dip', 'weighted_dip'],
        purpose: 'Dip strength foundation',
      },
      {
        // Strict / ring forms once the strict MU is owned.
        exerciseIds: ['strict_muscle_up', 'ring_muscle_up'],
        purpose: 'Strict / ring expression',
      },
    ],
    trunkSupport: {
      exerciseIds: ['hollow_body', 'hanging_leg_raise'],
      purpose: 'Tight body line through transition',
    },
  },
}
