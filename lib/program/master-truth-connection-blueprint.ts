...
function step22(): BlueprintPhase {
  ...
  return {
    ...
    status: 'PARTIAL',
    nextAction:
      'Step 22.1-22.5 COMPLETE. T.T9-T.T13 all complete. Post-workout proposal queue renders in pre-save completion UI. Step 22.5 second-confirmation flow allows user to explicitly apply substitution to saved program. No automatic mutation. Also pending: T.T7 Program Page advisory preview, T.T8 live workout advisory warning modal, reversal/undo for saved-program changes.',
    subtasks: [
      {
        id: 'T.T1',
      ...
      // [T.T13] Step 22.5 — Explicit saved-program mutation execution (COMPLETE)
      {
        id: 'T.T13',
        title: 'Step 22.5 explicit saved-program substitution apply',
        status: 'COMPLETE',
        evidence: [
          'Scoped proposal queue storage with programId/sessionId/dayKey scope',
          'ProposalQueueScope type enforces identity for saved-program apply eligibility',
          'saveScopedProposalQueue() persists to sessionStorage with scoped key',
          'updateAndPersistProposalStatus() persists dismiss/defer/accepted state immediately',
          'Legacy unscoped queue cannot apply to saved program (safeForSavedProgramApply: false)',
          'Stale queue expires after 24 hours',
          'SavedProgramSubstitutionApplyCandidate with canApply/blockedReason',
          'SavedProgramSubstitutionApplyConfirmation with secondConfirmationRequired: true',
          'SavedProgramSubstitutionApplyResult with exact proof metadata',
          'buildSavedProgramSubstitutionApplyCandidate() builds candidate from proposal',
          'buildSavedProgramSubstitutionConfirmation() builds second confirmation content',
          'canApplySavedProgramSubstitution() validates exact target match',
          'applySavedProgramSubstitutionToProgram() pure function mutates exact exercise only',
          'SavedProgramSubstitutionMetadata stamped on updated exercise',
          'Second confirmation UI in StreamlinedWorkoutSession post-workout review',
          'User must click "Use as Planned" then "Update Saved Program" for mutation',
          'Cancel keeps saved program unchanged',
          'Blocked/ambiguous targets show error, program unchanged',
          'saveAdaptiveProgram() used for persistence — existing safe corridor',
